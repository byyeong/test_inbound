...
/*function load_langpack($lang_path, $rpath)
{
	...
	$pos_file_st = -1;
	if ($source_size = filesize($lang_path)) {
		$fp_lang=fopen($lang_path,'rb');
		$cont=fread($fp_lang,$source_size);
		fclose($fp_lang);
		if (substr($cont,0,1) == 'F') $cont = "\n\n" . $cont;
		...
		while(true)
		{
			$pos = strpos($cont, "{\"", $lpos);
			if (!$pos) $pos = $size;
			$first = substr($cont, $lpos, $pos-$lpos);
			if ($pos_file_st == -1)
			{
				$pos_file_stx = strpos($first, $tofind);
				if ($pos_file_stx)
				{
					$pos_file_st = $pos_file_stx;
					$pos_file_st += $lpos;
					$pos_file_ed = strpos($first, "\nFILE: ", $pos_file_stx+3);
					if ($pos_file_ed)
					{
						$pos_file_ed += $lpos;
						break;
			...
			if ($pos == $size) break;
			$lpos = strpos($cont, "\"}", $pos);
			if (!$lpos) break;
				
				
		}
		
		if (!$pos_file_ed) $pos_file_ed = $size;
		if ($pos_file_st == -1) $pos_file_st = $pos_file_ed;
		$cont = substr($cont, $pos_file_st + 1, $pos_file_ed - $pos_file_st);
	}
...
function i18n_arrange($org_cont, $filename, $lang, $lang_name)
{
	$filename_real = "../tf_app/" . $filename;
	$source = '';
	if ($source_size = filesize($filename_real)) {
		$fp_tpl=fopen($filename_real,'rb');
		...
		foreach ($match[1] as $key=>$value) {
				$p1 = strpos($source, $match[0][$key], $p1);
				
		$before = substr($source, 0, $p1);
		$before = str_replace("\r\n", "\n", $before);
		$before_by_line = explode("\n", $before . " ");

		$linepos = sizeof($before_by_line);
		...
		// 사전 파일 읽어오기
		$pattern = "/^\s*([0-9]+):{\"(.*?)\"}\s*==>\s*{\"(.*?)\"}/ism"; 
		preg_match_all($pattern, $org_cont, $match);
		$replace_pattern = array();
		$p1 = 0;
		foreach ($match[1] as $key=>$value) {
			$p1 = strpos($org_cont, $match[0][$key], $p1);
				
			$before = substr($org_cont, 0, $p1);
			$before_by_line = explode("\n", $before . " ");
			$linepos = sizeof($before_by_line);
			
			$arr = array();
			$arr['line'] = $match[1][$key];
			$arr['src'] = $match[2][$key];
			$arr['to'] = $match[3][$key];
		...
		for ($i=0;$i<sizeof($org);$i++)
		{
			...
			{
				// cr/lf 형식 통일
				$org[$i]['cont'] = str_replace("\r\n", "\n", $org[$i]['cont']);
				$v['src'] = str_replace("\r\n", "\n", $v['src']);
				
				if ($org[$i]['cont'] == $v['src'])
				{//찾음
					if ($org[$i]['line'] == $v['line'])
					{//라인까지 같음
							$fixed = $k;
							break;
					}
			...
			else if ($first > -1 && array_key_exists($first, $org))
			{
				$tgt[$first]['line'] = $org[$first]['line'];
				$fixed_tgt[] = $tgt[$first];
				unset($tgt[$first]);
			}
			else
			{
				$tgt_temp = array();
				$tgt_temp['line'] = $org[$i]['line'];
				$tgt_temp['src'] = $org[$i]['cont'];
				$tgt_temp['to'] = "";
				$fixed_tgt[] = $tgt_temp;
			}
		...
		foreach ($tgt as $k=>$v)
		{
			$v['line'] = "DEPRECATED." . $v['line'];
			$fixed_tgt[] = $v;
		}
		return write_langpack($fixed_tgt, $lang, $filename, $lang_name);
		
	}
}

function write_langpack($tgt, $lang, $filename, $lang_name)
{
	$calculated_cont = "\nFILE: " . $filename . "\n";
	foreach ($tgt as $k=>$v)
	{
			$calculated_cont = $calculated_cont . $v['line'] . ":{\"" . $v['src'] . "\"}\n==>{\"" . $v['to'] . "\"}\n\n";
	}
	$lang_path = "../tf_app/languages/" . $lang . ".txt";
	...
	{
		$calculated_cont = "// Trail framework i18n dictionary\n// 트레일 프레임워크 국제화 사전 파일\n\n"
				. "// Language code: " . $lang . "\n"
				. "// Language: " . $lang_name . "\n\n\n" . $calculated_cont;
	}
	...
	$org_nakami = str_replace("\r\n", "\n", substr($cont, $pos_file_st, $pos_file_ed - $pos_file_st));
	$attachments = "";
	$temp_attach = "";
	$temp_resv = "";
	
	$phase = 0;
	for ($i=0;$i<strlen($org_nakami);$i++)
	{
		if ($phase == 0)
		{// 초기 상태 (기타 데이타)
			$temp_attach = $temp_attach . substr($org_nakami, $i, 1);
			
			if (substr($org_nakami,$i,1) == "\n")
			{// 개행상태
				$phase =1;
				continue;
			}
		...
		{ //개행 이후 숫자
			$temp_resv = $temp_resv . substr($org_nakami, $i, 1);
			if (
					trim(substr($org_nakami,$i,1)) == "" || 
					(substr($org_nakami,$i,1) >= "0" && substr($org_nakami,$i,1) <= "9"
					)
					)
			{
				continue;
			}
			else if (trim(substr($org_nakami,$i,4)) == "FILE")
			{
				$phase = 3;
				continue;
			}
			else if (trim(substr($org_nakami,$i,10)) == "DEPRECATED")
			{
				$i = $i + 9;
				$phase = 4;
			}
			else if (substr($org_nakami,$i,1) == ":")
			{// 커마
				$phase = 2;
				continue;
			}
			...
			{
				$temp_attach = $temp_attach . $temp_resv;
				$temp_resv = "";
				$phase = 0;
			}
		}   
		...
		{// 개행 이후 숫자 이후
			if (trim(substr($org_nakami,$i,1)) == "")
			{
				continue;
			}
			if (substr($org_nakami,$i,2) == "{\"")
			{
				if (trim($temp_attach) != "")
				{
					$attachments = $attachments . trim($temp_attach) . "\n";
				}
	...
	$attachments = trim($attachments . trim($temp_attach . $temp_resv));
	...
	if (!$pos_file_ed) $pos_file_ed = $size;
	if ($pos_file_st == -1) $pos_file_st = $pos_file_ed;
	$cont_first = substr($cont, 0, $pos_file_st);
	$cont_last = substr($cont, $pos_file_ed);
	
	if (sizeof($tgt) || $attachments)
			$cont = $cont_first . $calculated_cont . $attachments . $cont_last;
	else
			$cont = $cont_first . $cont_last;

	$fp_cpl=fopen($lang_path, 'wb');
	fwrite($fp_cpl, $cont);
	fclose($fp_cpl);
...
{
	//var $langpack_loaded = false;
	//var $langpack_cont = "";
	
	//var $auto_arranged = false;
	/*
	function i18n_compile($org, $pos)
	{
		$org = str_replace("\r\n", "\n", $org);
		
		if (!$this->langpack_loaded)
		{
			$this->langpack_cont = load_langpack("../tf_app/languages/" . $this->lang . ".txt", $this->rpath);
			$this->langpack_loaded;
		}
		
		$tofind = $pos . ":" . "{\"" . $org . "\"}";
		$pos = strpos($this->langpack_cont, $tofind);
		
		if ($pos)
		{
			$pos_start = strpos($this->langpack_cont, "=>{\"", $pos+1);
	
			if ($pos_start)
				$pos_end = strpos($this->langpack_cont, "\"}", $pos_start);
			
			
			if ($pos_start && $pos_end)
			{
				$cont = substr($this->langpack_cont, $pos_start + 4, $pos_end - $pos_start - 4);
			}
			else
			{
				$cont = $org;
		...
		{
			$cont = $org;
			if (!$this->auto_arranged)
			{
				$this->auto_arrange();
				$cont = $this->i18n_compile($org, $pos);
			}
		}
	...
	{
		$this->auto_arranged = true;
		for ($i=5;$i>0;$i--)
		{
			if (file_exists("../tf_app/languages/backup/" . $this->lang . ".txt." . $i))
			{
				copy("../tf_app/languages/backup/" . $this->lang . ".txt." . $i, "../tf_app/languages/backup/" . $this->lang . ".txt." . ($i+1));
			}
				
		}
		
		if (file_exists("../tf_app/languages/" . $this->lang . ".txt"))
		{
			copy("../tf_app/languages/" . $this->lang . ".txt", "../tf_app/languages/backup/" . $this->lang . ".txt.1");
		}
		
		if (!array_key_exists("lang_name", $_SESSION)) $_SESSION["lang_name"] = "en";
		$this->langpack_cont = i18n_arrange($this->langpack_cont, $this->rpath, $this->lang, $_SESSION["lang_name"]);
    ...
    */
	...
	function _compile_template(&$tpl, $tpl_path, $cpl_base, $cpl_head)
	{
		...
		{
			if (substr($v,0,$ns1_len) == $ns1)
			{
				$functions['user'][$k] = substr($v, $ns1_len);
			}
		}
...
?>
		치명적인 에러: 탬플릿 캐시 폴더가 존재하지 않거나, 쓰기 불가능으로 되어 있는 것 같습니다.<br />
		<br />
		<?=$cwd?> 위치에 template.cache 폴더가 있는지 확인하고,<br />
		웹 유저에게 쓰기 권한이 부여되어 있는지 확인 해 주시기 바랍니다.<br /><br />
		더 이상 실행될 수 없습니다. 종료됩니다.<br />
<?php
						$this->exit_();
					}
					if (!$this->on_ms) {
	...
	// get template
		...
		if ($source_size = filesize($tpl_path)) {
            ...
            /*
						...
						$replace_pattern = array();
						foreach ($match[1] as $key=>$value) {
								$p1 = strpos($source, $match[0][$key]);
								
					$before = substr($source, 0, $p1);
					$before = str_replace("\r\n", "\n", $before);
					$before_by_line = explode("\n", $before . " ");
					$linepos = sizeof($before_by_line);
					$compiled = $this->i18n_compile($value, $linepos);
								
								$source = str_replace($match[0][$key], $compiled, $source);
						}
                        */
			// end of i18n
	...
	// parse template
		...
		$hdr = "\nnamespace userstory\\trail;";
		$this->_save_result($cpl_path, $cpl_head, ' */ ' . $hdr.$plugins.$size_of_top_loop.'?>'.$nl, $source);
	...
	function _save_result($cpl_path, $cpl_head, $init_code, $source)
	{
		...
		$hdr = '<' . '?php namespace userstory\\trail;?' . '>';
...
<?php
	...
	function report($type, $msg, $file=false, $line=false)
	{
		...
		//echo $report."</span><br />\n";
		//Trail::get_instnace()->
		$file = is_string($file)?$file:$this->tpl_path;
		
        include_once dirname(__file__).'/TemplateCompileException.php';

		throw new TemplateCompileException($file, $msg, $line_final);
	}
	function exit_()
	{
